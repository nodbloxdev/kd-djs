...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!message.member.hasPermission("BAN_MEMBERS")) {
            return message.reply("❌ You do not have permissions to ban members. Please contact a(n) HR member")
                .then(m => m.delete(5000));
        ...
        if (toBan.id === message.author.id) {
            return message.reply("Oops! You ban yourself!")
                .then(m => m.delete(5000));
        }

        // Check if the user's banable
        if (!toBan.bannable) {
            return message.reply("Sadly, I cannot ban this user.. :/")
                .then(m => m.delete(5000));
        }
